...
class StatCache {
    ...
    public function getStaticPath($resource, array $options = array()) {
        ...
        // ebc: replaced '$resource->Context' with '$context' throughout file
        $context = $this->modx->getContext( $resource->get('context_key') );
      
        $path = $context->getOption('statcache_path', MODX_BASE_PATH . 'statcache', $options);

        /* generate an absolute URI representation of the Resource to append to the path */
        if ($resource->get('id') === (integer)$context->getOption('site_start', 1, $options)) {
            $uri  = $context->getOption('base_url', MODX_BASE_URL, $options);
            /* use ~index.html to represent the site_start Resource */
            $uri .= '~index.html';
        } else {
            $uri = $this->modx->makeUrl($resource->get('id'), $resource->get('context_key'), '', 'abs');
            if (strpos($uri, $context->getOption('url_scheme') . $context->getOption('http_host')) === 0) {
                /* remove url_scheme and http_host from any full URLs generated by MODX automatically */
                $uri = substr($uri, strlen($context->getOption('url_scheme') . $context->getOption('http_host')));
            }
        ...
        if ($this->getOption('use_url_scheme', false)) {
            $path .= '/' . str_replace('://', '', $context->getOption('url_scheme'));
        }
        if ($this->getOption('use_http_host', false)) {
            $path = rtrim($path, '/') . '/' . $context->getOption('http_host');
        }
